...
from featherstone_py.spatial import colvec, Tinv, Ad
from featherstone_py.utils import take_last


def apply_external_forces(
    f_ext: Optional[Dict[int, np.ndarray]],
...
def apply_end_effector_exerted_force(
    f_tip: Optional[np.ndarray], model: MultibodyModel, F: Dict[int, np.ndarray]
) -> Dict[int, np.ndarray]:
    """
    ...
    if model.T_n_ee is None:
        raise ValueError(
            "End-effector transform is not set can't apply end-effector exerted force"
        )

    F_out = F.copy()
    ee_force = take_last(F_out)
    X_ee_n = Ad(Tinv(model.T_n_ee))
    ee_force += X_ee_n.T @ colvec(f_tip)
