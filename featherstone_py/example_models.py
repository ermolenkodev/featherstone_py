...
from featherstone_py.spatial import (
    colvec,
    T,
...
class DoublePendulum:
    ...
    def __init__(self, l0=0.5, l1=0.7, l2=0.4, m=1.0, r=0.05, w=0.08, h=0.06):
        ...
        self.link2_central_inertia = [
            m * (w ** 2 + l2 ** 2) / 12,
            m * (h ** 2 + l2 ** 2) / 12,
            m * (h ** 2 + w ** 2) / 12,
        ]

    def to_modern_robotics_notation(
        self,
        ...
        Slist = np.hstack(
            [colvec([1, 0, 0, 0, l0 / 2, 0]), colvec([1, 0, 0, 0, l0 / 2 + l1, 0])]
        )
        ...
        Glist = np.array(
            [
                centroidal_inertia(rotational_inertia(self.link1_central_inertia), m),
                centroidal_inertia(rotational_inertia(self.link2_central_inertia), m),
            ]
        )
    ...
    def to_featherstone_notation(self) -> MultibodyModel:
        ...
        T_n_ee = T(R=np.eye(3), p=colvec([0, 0, l2]))

        return MultibodyModel(n_bodies, joints, parent, X_tree, I, T_n_ee)
