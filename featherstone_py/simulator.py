...
from featherstone_py.forward_dynamics import (
    InertiaMatrixMethod,
    InverseDynamicsUsingRNEA,
)
...
class Simulator:
    ...
    def advance_to(
        self, q_: np.ndarray, qd_: np.ndarray, tau_: np.ndarray, time: float
        ...
        while self.curr_time < time:
            qdd = self.forward_dynamics(
                self.model, q, qd, tau, gravity=self.gravity
            ).reshape(q_.shape)
            q += qd * dt
            qd += qdd * dt
