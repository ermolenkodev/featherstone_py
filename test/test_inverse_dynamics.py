...
from featherstone_py.spatial import Ad, colvec
...
def test_torques_calculation(double_pendulum_models, q, qd, qdd, g, Ftip):
    ...
    np.testing.assert_allclose(
        rnea(m_feath_notation, q, qd, qdd, g)[:, 0],
...
@pytest.mark.parametrize(
    "q,qd,qdd,g,Ftip",
    [([0.1, 0.1], [0.1, 0.2], [2, 1.5], [0, 0, -9.81], np.zeros(6)),
     ([0.2, 0.2], [0., 0.], [0.1, 0.], [0, 0, -9.81], np.zeros(6))]
)
def test_wrenches_calculation(double_pendulum_models, q, qd, qdd, g, Ftip):
    ...
    algorithm = RNEAImpl()
    algorithm.run(m_feath_notation, q, qd, qdd, g)
